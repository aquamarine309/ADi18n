...
export class DimBoost {
  ...
  static get unlockedByBoost() {
    ...
    if (!allNDUnlocked && boosts < DimBoost.maxDimensionsUnlockable - 4) {
      newUnlock = $t("dimboost_button_unlock_dimension", AntimatterDimension(boosts + 5).name);
    } else if (boosts === 4 && !NormalChallenge(10).isRunning && !EternityChallenge(3).isRunning) {
      newUnlock = $t("dimboost_button_unlock_sacrifice");
    }
    const formattedMultText = formatX(DimBoost.power, 2, 1);
    let dimensionRange;
    if (boosts <= 0) dimensionRange = $t("dimboost_button_boost_first_dimension", formattedMultText);
    else dimensionRange = $t("dimboost_button_boost_range_of_dimension", formattedMultText, "1", Math.min(boosts + 1, 8).toString());
    if (boosts >= DimBoost.maxDimensionsUnlockable - 1) dimensionRange = $t("dimboost_button_boost_all_dimension", formattedMultText);

    let boostEffects;
    if (NormalChallenge(8).isRunning) boostEffects = newUnlock;
    else if (newUnlock === "") boostEffects = dimensionRange;
    else boostEffects = $t("X_and_Y", newUnlock, dimensionRange);

    if (boostEffects === "") return $t("dimboost_button_currently_useless");
    ...
    return $t("dimboost_button_reset_dimensions", boostEffects);
  }
