...
(${this.exportDateString}).txt`, GameSaveSerializer.serialize(backupData));
  ...
  exportModifiedSave() {
    ...
    player.celestials.enslaved = deepmergeAll([{}, player.celestials.nameless]);
    player.celestials.ra.pets.enslaved = deepmergeAll([{}, player.celestials.ra.pets.nameless]);
  ...
  loadPlayerObject(playerObject) {
    ...
    if (player.celestials.enslaved) {
      player.celestials.nameless = deepmergeAll([{}, player.celestials.enslaved]);
      player.celestials.ra.pets.nameless = deepmergeAll([{}, player.celestials.ra.pets.enslaved]);
    }
    ...
    if (simulateOffline && !Speedrun.isPausedAtStart()) {
      ...
      if (diff > 5 * 60 * 1000 && player.celestials.nameless.autoStoreReal) {
        diff = Nameless.autoStoreRealTime(diff);
      }
      if (diff > 10000) {
  ...
  postLoadStuff() {
    ...
    Nameless.nextTickDiff = player.options.updateRate;
