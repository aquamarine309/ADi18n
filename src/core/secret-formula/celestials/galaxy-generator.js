...
export const pelleGalaxyGeneratorUpgrades = {
  additive: rebuyable({
    ...
    description: () => $t("galaxy_generator_upgrade_0_description"),
    cost: x => Math.pow(3, x),
    effect: x => x * 2,
    formatEffect: x => `${format(x, 2, 2)}/s`,
    currency: () => Currency.galaxyGeneratorGalaxies,
    currencyLabel: "galaxies"
  }),
  multiplicative: rebuyable({
    id: "galaxyGeneratorMultiplicative",
    description: () => $t("galaxy_generator_upgrade_1_description"),
