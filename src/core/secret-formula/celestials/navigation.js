...
export function vUnlockLegendLabel(complete, index) {
  ...
  return `到达 ${db.format(db.resource())} / ${db.format(db.requirement)} ${db.name}.`;
}
...
const Positions = Object.freeze({
  ...
  namelessReality: new Vector(650, 250),
  namelessGlyphLevel: new Vector(650 + 75 * Math.cos(Math.PI / 180 * -60), 250 + 75 * Math.sin(Math.PI / 180 * -60)),
  namelessGlyphRarity: new Vector(650 + 75 * Math.cos(Math.PI / 180 * 120), 250 + 75 * Math.sin(Math.PI / 180 * 120)),
  ...
  raPetNameless: new Vector(400 + 85 * Math.sin(Math.PI / 180 * 78), 200 + 85 * Math.cos(Math.PI / 180 * 78)),
...
export const celestialNavigation = {
    ...
    node: {
      ...
      legend: {
        ...
        text: () => {
          ...
          return `倒入 ${format(rm, 2)} / ${format(cost, 2)} 现实机器`;
        },
        angle: 135,
        diagonal: 16,
        horizontal: 16,
    ...
    node: {
      ...
      legend: {
        text: complete => {
          ...
          return [
            "特蕾莎的复兴商店",
            `倒入 ${format(rm, 2)} / ${format(cost, 2)} 现实机器`
          ];
        },
        angle: -35,
        diagonal: 16,
    ...
    node: {
      ...
      legend: {
        text: complete => {
          ...
          if (complete === 0) return "解锁鹿颈长的无限";
          const am = Effarig.isRunning ? Currency.antimatter.value : 0;
          return [
            "鹿颈长的无限",
            "在鹿颈长的现实中达到"
            `${format(am, 2)} / ${format(Number.MAX_VALUE, 2)} 反物质`,
          ];
        },
        angle: 0,
        diagonal: 100,
      ...
      legend: {
        text: complete => {
          ...
          return [
            "鹿颈长的永恒",
            "在鹿颈长的现实中达到",
            `${format(ip, 2)} / ${format(Number.MAX_VALUE, 2)} 无限点数`,
          ];
        },
        angle: -45,
        diagonal: 16,
  ...
  "effarig-reality": {
    ...
    node: {
      ...
      legend: {
        text: complete => {
          if (complete >= 1) return $t("effarig_reality");
          const ep = Effarig.isRunning ? Currency.eternityPoints.value : 0;
          const goal = DC.E4000;
          return [
            $t("effarig_reality"),
            "在鹿颈长的现实中达到",
            `${format(ep, 2)} / ${format(goal, 2)} 永恒点数`,
          ];
        },
        angle: -120,
        diagonal: 82,
  ...
  "nameless": {
    ...
    drawOrder: -1,
    node: {
      clickAction: () => Tab.celestials.nameless.show(true),
      incompleteClass: "c-celestial-nav__test-incomplete",
      fill: "#ffa337",
      position: Positions.namelessReality,
      ring: {
        rMajor: 80,
        rMinor: 70,
        gapCenterDeg: 15,
    ...
    connector: {
      ...
      path: LinearPath.connectCircles(Positions.effarigNode, 40 - 1, Positions.namelessReality, 80 - 1),
      fill: "url(#gradEffarigNameless)",
    }
  },
  "nameless-unlock-glyph-level": {
    visible: () => EffarigUnlock.eternity.isUnlocked,
    ...
    node: {
      ...
      incompleteClass: "c-celestial-nav__test-incomplete",
      fill: "#ffa337",
      position: Positions.namelessGlyphLevel,
      ring: {
        rMajor: 24,
        rMinor: 16,
        gapCenterDeg: 40,
      ...
      legend: {
        text: complete => {
          if (complete >= 1) return "已破坏符文等级锁链";
          const goal = 5000;
          return [
            "破坏锁链",
            `达到 ${formatInt(Math.min(player.records.bestReality.glyphLevel, goal))}/${formatInt(goal)} 符文等级`
          ];
        },
        angle: -45,
        diagonal: 16,
  ...
  "nameless-unlock-glyph-rarity": {
    ...
    node: {
      ...
      incompleteClass: "c-celestial-nav__test-incomplete",
      fill: "#ffa337",
      position: Positions.namelessGlyphRarity,
      ring: {
        rMajor: 24,
        rMinor: 16,
        gapCenterDeg: 220,
      ...
      legend: {
        text: complete => {
          if (complete >= 1) return "已破坏符文稀有度锁链";
          const goal = 100;
          return [
            "破坏锁链",
            `到达${formatPercents(complete * goal / 100, 1)}/${formatPercents(goal / 100, 1)} 符文稀有度`
          ];
        },
        angle: 45,
        diagonal: 32,
    ...
    connector: {
      pathStart: 0,
      pathEnd: 1,
      path: new LinearPath(Positions.namelessGlyphRarity, Positions.namelessGlyphLevel).trimStart(23).trimEnd(23),
      fill: "#ffa337",
      completeWidth: 6,
      incompleteWidth: 4,
    }
  ...
  "nameless-reality": {
    visible: () => EffarigUnlock.eternity.isUnlocked,
    complete: () => {
      if (Nameless.isCompleted) return 1;
    ...
    node: {
      ...
      legend: {
        text: complete => {
          if (complete >= 1) return $t("nameless_reality");
          const ep = Nameless.isRunning ? Currency.eternityPoints.value : 0;
          const goal = DC.E4000;
          return [
            $t("nameless_reality"),
            "在无名氏的现实中达到",
    ...
    connector: {
      pathStart: 0,
      pathEnd: 1,
      path: new LinearPath(Positions.namelessGlyphRarity, new Vector(650 + 74 * Math.sqrt(0.75), 250 + 74 * 0.5))
        .trimStart(23),
      fill: "#ffa337",
    }
  },
  "v-unlock-achievement": {
    ...
    node: {
      ...
      legend: {
        text: complete => {
          const goal = 800;
          if (complete >= 1) return $t("v_reality");
    ...
    connector: {
      pathStart: 0,
      pathEnd: 1,
      path: LinearPath.connectCircles(Positions.namelessReality, 80 - 1, Positions.vUnlockAchievement, 16 - 1),
      fill: "url(#gradNamelessV)",
      completeWidth: 6,
      incompleteWidth: 4,
    }
  },
    ...
    node: {
      ...
      legend: {
        text: complete => {
          ...
          return [
            "薇成就",
            `完成 ${name} ${formatInt(completions)} / ${formatInt(6)} 次`
          ];
        },
        angle: -135,
        diagonal: 16,
  ...
  "nameless-pet": {
    ...
    node: {
      ...
      fill: "#9063de",
      isStacked: true,
      position: Positions.raPetNameless,
      ring: {
        rMajor: 12,
      },
      legend: {
        text: complete => {
          ...
          const level = Ra.pets.nameless.level;
    ...
    connector: {
      pathStart: 0.05,
      pathEnd: 0.95,
      path: new LinearPath(Positions.raReality, Positions.raPetNameless),
      fill: "#9063de",
      completeWidth: 6,
      incompleteWidth: 4,
    }
  },
  "nameless-pet-to-nameless": {
    visible: () => Ra.unlocks.namelessUnlock.isUnlocked,
    complete: () => Ra.pets.nameless.level / 25,
    drawOrder: -1,
    connector: {
      pathStart: 0.05,
      pathEnd: 0.55,
      path: new LinearPath(Positions.raPetNameless, Positions.namelessReality),
      fill: "url(#gradRaNameless)",
      completeWidth: 6,
      incompleteWidth: 4,
    }
  },
  "v-pet": {
    visible: () => Ra.unlocks.namelessUnlock.isUnlocked,
    complete: () => Ra.pets.nameless.level / 8,
    drawOrder: -1,
    node: {
      clickAction: () => Tab.celestials.ra.show(true),
      incompleteClass: "c-celestial-nav__test-incomplete",
      ...
      legend: {
        text: complete => {
          const unlocked = Ra.pets.nameless.level;
  ...
  "laitela-unlock": {
    ...
    node: {
      ...
      legend: {
        text: complete => {
          const realityName = $t("laitela_reality");
    ...
    node: {
      ...
      legend: {
        text: complete => {
          ...
          let laitelaString = `${format(Currency.eternityPoints.value)} / ${format("1e4000")} ${$t("eternity_points_short")}`;
          ...
          return [
            "解锁佩勒",
            "反物质之神",
            `${format(Currency.imaginaryMachines.value, 2)} / ${format(1.6e15, 2)} ${$t("imaginary_machines_short")}`,
            laitelaString
          ];
        },
        angle: 105,
  ...
  "pelle-doomed-requirement": {
    ...
    node: {
      ...
      legend: {
        text: complete => {
          if (complete >= 1) return Pelle.isDoomed ? $t("doomed_reality") : $t("doom_your_reality");
          const achievements = [Achievements.prePelleRows.countWhere(r => r.every(a => a.isUnlocked)),
            Achievements.prePelleRows.length];
          const alchemy = [AlchemyResources.all.countWhere(r => r.capped), AlchemyResources.all.length];
          return [
