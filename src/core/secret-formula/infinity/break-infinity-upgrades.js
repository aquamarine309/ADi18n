import { DC } from "../../constants.js";

function rebuyable(config) {
  const effectFunction = config.effect || (x => x);
  const { id, maxUpgrades, description, isDisabled, noLabel, onPurchased } = config;
  return {
    rebuyable: true,
    id,
    cost: () => config.initialCost * Math.pow(config.costIncrease, player.infinityRebuyables[config.id]),
    maxUpgrades,
    description,
    effect: () => effectFunction(player.infinityRebuyables[config.id]),
    isDisabled,
    // There isn't enough room in the button to fit the EC reduction and "Next:" at the same time while still
    // presenting all the information in an understandable way, so we only show it if the upgrade is maxed
    formatEffect: config.formatEffect ||
      (value => {
        const afterECText = config.afterEC ? config.afterEC() : "";
        return value === config.maxUpgrades
          ? `${$t("currently")}: ${formatX(10 - value)} ${afterECText}`
          : `${$t("currently")}: ${formatX(10 - value)} | ${$t("next")}: ${formatX(10 - value - 1)}`;
      }),
    formatCost: value => format(value, 2, 0),
    noLabel,
    onPurchased
  };
}

export const breakInfinityUpgrades = {
  totalAMMult: {
    id: "totalMult",
    cost: 1e4,
    description: () => $t("break_infinity_upgrade_0"),
    effect: () => Math.pow(player.records.totalAntimatter.exponent + 1, 0.5),
    formatEffect: value => formatX(value, 2, 2)
  },
  currentAMMult: {
    id: "currentMult",
    cost: 5e4,
    description: () => $t("break_infinity_upgrade_1"),
    effect: () => Math.pow(Currency.antimatter.exponent + 1, 0.5),
    formatEffect: value => formatX(value, 2, 2)
  },
  galaxyBoost: {
    id: "postGalaxy",
    cost: 5e11,
    description: () => $t("break_infinity_upgrade_7", formatPercents(0.5)),
    effect: 1.5
  },
  infinitiedMult: {
    id: "infinitiedMult",
    cost: 1e5,
    description: () => $t("break_infinity_upgrade_2"),
    effect: () => 1 + Currency.infinitiesTotal.value.pLog10() * 10,
    formatEffect: value => formatX(value, 2, 2)
  },
  achievementMult: {
    id: "achievementMult",
    cost: 1e6,
    description: () => $t("break_infinity_upgrade_3"),
    effect: () => Math.max(Math.pow((Achievements.effectiveCount - 30), 3) / 40, 1),
    formatEffect: value => formatX(value, 2, 2)
  },
  slowestChallengeMult: {
    id: "challengeMult",
    cost: 1e7,
    description: () => $t("break_infinity_upgrade_4"),
    effect: () => Decimal.clampMin(50 / Time.worstChallenge.totalMinutes, 1),
    formatEffect: value => formatX(value, 2, 2),
    hasCap: true,
    cap: DC.D3E4
  },
  infinitiedGen: {
    id: "infinitiedGeneration",
    cost: 2e7,
    description: () => $t("break_infinity_upgrade_5"),
    effect: () => player.records.bestInfinity.time,
    formatEffect: value => {
      if (value === Number.MAX_VALUE && !Pelle.isDoomed) return $t("break_infinity_upgrade_5_no_effect");
      let infinities = DC.D1;
      infinities = infinities.timesEffectsOf(
        RealityUpgrade(5),
        RealityUpgrade(7),
        Ra.unlocks.continuousTTBoost.effects.infinity
      );
      infinities = infinities.times(getAdjustedGlyphEffect("infinityinfmult"));
      const timeStr = Time.bestInfinity.totalMilliseconds <= 50
        ? `${TimeSpan.fromMilliseconds(100).toStringShort()} (${$t("capped")})`
        : `${Time.bestInfinity.times(2).toStringShort()}`;
      return $p("break_infinity_upgrade_5_effect", infinities, format(infinities), timeStr);
    }
  },
  autobuyMaxDimboosts: {
    id: "autobuyMaxDimboosts",
    cost: 5e9,
    description: () => $t("break_infinity_upgrade_6"),
  },
  autobuyerSpeed: {
    id: "autoBuyerUpgrade",
    cost: 1e15,
    description: () => $t("break_infinity_upgrade_8")
  },
  tickspeedCostMult: rebuyable({
    id: 0,
    initialCost: 1e6,
    costIncrease: 5,
    maxUpgrades: 8,
    description: () => $t("break_infinity_upgrade_9"),
    afterEC: () => (EternityChallenge(11).completions > 0
      ? $t("break_infinity_upgrade_9_10_effect", $t("ec_name_short", "11"), formatX(Player.tickSpeedMultDecrease, 2, 2))
      : ""
    ),
    noLabel: true,
    onPurchased: () => GameCache.tickSpeedMultDecrease.invalidate()
  }),
  dimCostMult: rebuyable({
    id: 1,
    initialCost: 1e7,
    costIncrease: 5e3,
    maxUpgrades: 7,
    description: () => $t("break_infinity_upgrade_10"),
    afterEC: () => (EternityChallenge(6).completions > 0
      ? $t("break_infinity_upgrade_9_10_effect", $t("ec_name_short", "6"), formatX(Player.dimensionMultDecrease, 2, 2))
      : ""
    ),
    noLabel: true,
    onPurchased: () => GameCache.dimensionMultDecrease.invalidate()
  }),
  ipGen: rebuyable({
    id: 2,
    initialCost: 1e7,
    costIncrease: 10,
    maxUpgrades: 10,
    effect: value => Player.bestRunIPPM.times(value / 20),
    description: () => {
      if (BreakInfinityUpgrade.ipGen.isCapped) {
        return $t("break_infinity_upgrade_11", formatInt(5 * (1 + player.infinityRebuyables[2])), formatInt(10));
      }
      return $t("break_infinity_upgrade_11_b", formatInt(0.05 * player.infinityRebuyables[2]), formatPercents(0.05 * (1 + player.infinityRebuyables[2])), formatInt(10))
    },
    isDisabled: effect => effect.eq(0),
    formatEffect: value => $t("break_infinity_upgrade_11_effect", format(value, 2, 1)),
    noLabel: false
  })
};
