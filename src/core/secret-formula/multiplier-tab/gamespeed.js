...
export const gamespeed = {
  total: {
    name: "Game speed",
    displayOverride: () => {
      if (Nameless.isStoringRealTime) return `Set to ${format(0)} (storing real time)`;
      if (EternityChallenge(12).isRunning) return $t("X_fixed", `${formatX(1)}/${formatInt(1000)}`);
      ...
      const avgSpeed = Nameless.isAutoReleasing
  ...
  pulsing: {
    ...
    multValue: () => (Nameless.isAutoReleasing
      ? Math.max(Nameless.autoReleaseSpeed / getGameSpeedupFactor(), 1)
      : getGameSpeedupFactor()),
    isActive: () => Nameless.canRelease() && Nameless.isAutoReleasing && !EternityChallenge(12).isRunning,
  ...
  chargingBH: {
    ...
    isActive: () => Nameless.isStoringGameTime,
    icon: MultiplierTabIcons.BLACK_HOLE,
  },
  invertedBH: {
