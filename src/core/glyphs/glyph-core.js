...
export const Glyphs = {
  ...
  saveUndo(targetSlot) {
    const undoData = {
      ...
      storedTime: player.celestials.nameless.stored,
  ...
  undo() {
    ...
    player.celestials.nameless.stored = undoData.storedTime || 0;
...
export function getAdjustedGlyphLevel(glyph, realityGlyphBoost = Glyphs.levelBoost, ignoreCelestialEffects = false) {
  ...
  if (!ignoreCelestialEffects) {
    ...
    if (Nameless.isRunning) return Math.max(level, Nameless.glyphLevelMin);
