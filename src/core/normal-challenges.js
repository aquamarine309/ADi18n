...
export function updateNormalAndInfinityChallenges(diff) {
  if (NormalChallenge(11).isRunning || InfinityChallenge(6).isRunning) {
    ...
    if (Currency.matter.gt(Currency.antimatter.value) && NormalChallenge(11).isRunning && !Player.canCrunch) {
      ...
      Modal.message.show($t("matter_annihilation_message", format(values[0], 2, 2), format(values[1], 2, 2)), { closeEvent: GAME_EVENT.BIG_CRUNCH_AFTER }, 1);
    }
  }
...
class NormalChallengeState extends GameMechanicState {
  ...
  start() {
    ...
    if (Nameless.isRunning && EternityChallenge(6).isRunning && this.id === 10) {
      NamelessProgress.challengeCombo.giveProgress();
      Nameless.quotes.ec6C10.show();
    }
    if (!Nameless.isRunning) Tab.dimensions.antimatter.show();
  }
  ...
  get goal() {
    if (Nameless.isRunning && Nameless.BROKEN_CHALLENGES.includes(this.id)) {
      return DC.E1E15;
    }
  ...
  get name() {
    return $t("nc_name", this.id.toString());
  }
}
