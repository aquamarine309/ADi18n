...
class AchievementState extends GameMechanicState {
  ...
  get languageId() {
    return this.config.languageId ?? this.id;
  }

  get name() {
    return $t(`achievement_${this.languageId}_name`);
  }
  ...
  unlock(auto) {
    ...
    if (this.id === 55 && !PlayerProgress.realityUnlocked()) {
      Modal.message.show(`由于你的大坍缩时间进了一分钟，屏幕上的部分UI发生了些许改变。
        现在维度页面会在大坍缩按钮出现时仍然显示。大坍缩按钮会悬浮在维度页面的上方。
        这纯粹是视觉上的优化，是为了防止闪烁。`, {}, 3);
    }
