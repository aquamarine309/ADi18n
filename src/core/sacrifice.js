...
export class Sacrifice {
  ...
  static get disabledCondition() {
    if (NormalChallenge(10).isRunning) return $t("sacrifice_locked_reason_8th_dimension");
    if (EternityChallenge(3).isRunning) return $t("ec_name", "3");
    if (DimBoost.purchasedBoosts < 5) return $t("sacrifice_locked_reason_dimboost", formatInt(5));
    if (AntimatterDimension(8).totalAmount.eq(0)) return $t("sacrifice_locked_reason_8th_dimension");
    if (this.nextBoost.lte(1)) return `${formatX(1)} 加成`;
    if (Player.isInAntimatterChallenge) return $t("sacrifice_locked_reason_reached_goal");
    return $t("sacrifice_locked_reason_need_to_crunch");
  }
