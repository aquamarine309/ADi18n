...
export const AutomatorCommands = [
    ...
    validate: (ctx, V) => {
      ...
      if (ctx.PrestigeEvent[0].tokenType === T.Infinity) {
        ...
        if (advSetting && !EternityMilestone.bigCrunchModes.isReached) {
          V.addError((ctx.duration || ctx.xHighest)[0],
            $t("advanced_infinity_autobuyer_not_unlocked"),
      ...
      if (ctx.PrestigeEvent[0].tokenType === T.Eternity) {
        ...
        if (advSetting && !RealityUpgrade(13).isBought) {
          V.addError((ctx.duration || ctx.xHighest)[0],
            $t("advanced_eternity_autobuyer_settings_not_unlocked"),
  ...
  },
  {
    ...
    compile: ctx => {
      ...
      return () => {
        ...
        } else if (Nameless.isRunning || Pelle.isDisabled("blackhole")) {
  ...
  },
  {
    ...
    compile: ctx => {
      ...
      const on = Boolean(ctx.On);
      return () => {
        if (on !== player.celestials.nameless.isStoring) Nameless.toggleStoreBlackHole();
      ...
      return {
        run: S => {
          ...
          if (S.commandState.prestigeLevel >= prestigeLevel) {
            AutomatorData.logCommandEvent($t("X_prestige_has_occurred_exiting_until_loop", prestigeName),
              ctx.startLine);
            return AUTOMATOR_COMMAND_STATUS.NEXT_INSTRUCTION;
          }
          AutomatorBackend.push(commands);
          ...
          $t(
            "X_prestige_has_not_occurred_yet_moving_to_line_Y_start_of_until_loop",
            prestigeName,
  ...
  },
  {
    ...
    compile: ctx => () => {
      ...
      const bhStr = off ? "inactive Black Holes" : $t("active_black_hole_X", holeID);
      if (bhCond) {
        const timeWaited = TimeSpan.fromMilliseconds(Date.now() - AutomatorData.waitStart).toStringShort();
