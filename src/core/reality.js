...
export function simulatedRealityCount(advancePartSimCounters) {
  const amplifiedSim = Nameless.boostReality ? Nameless.realityBoostRatio - 1 : 0;
...
function giveRealityRewards(realityProps) {
  ...
  if (multiplier > 1 && Nameless.boostReality) {
    ...
    if (Time.thisRealityRealTime.totalSeconds < 1) {
      player.celestials.nameless.storedReal *= 1 - Time.thisRealityRealTime.totalSeconds;
    } else {
      player.celestials.nameless.storedReal = 0;
    }
  ...
  if (Nameless.isRunning) Nameless.completeRun();
...
function restoreCelestialRuns(celestialRunState) {
  ...
  player.celestials.nameless.run = celestialRunState.nameless;
  if (player.celestials.nameless.run) Nameless.initializeRun();
...
export function clearCelestialRuns() {
  const saved = {
    ...
    nameless: player.celestials.nameless.run,
  ...
  if (Nameless.isRunning) {
    player.celestials.nameless.run = false;
    if (Tabs.current.isHidden || Tabs.current._currentSubtab.isHidden) Tab.celestials.nameless.show();
