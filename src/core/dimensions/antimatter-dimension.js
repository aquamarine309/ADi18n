...
export function buyOneDimension(tier) {
  ...
  if (tier === 8 && Nameless.isRunning && AntimatterDimension(8).bought >= 1) return false;
...
export function buyManyDimension(tier) {
  ...
  if (tier === 8 && Nameless.isRunning) return buyOneDimension(8);
...
class AntimatterDimensionState extends DimensionState {
   ...
   */
  get name() {
    return $t(`_${this.shortDisplayName}_ad`);
  }
  ...
  get continuumValue() {
    ...
    if (this.tier === 8 && Nameless.isRunning) return 1;
