window.TimeSpan = class TimeSpan {
  ...
  toString() {
    if (this.years > 1e6) {
      return `${format(this.totalYears, 3, 0)} ${$p("year", this.totalYears)}`;
    }
  ...
  toStringNoDecimals() {
    ...
    function addComponent(value, name) {
      parts.push(`${formatInt(value)} ${$p(name, value)}`);
    }
    ...
    if (parts.length === 0) return `${formatInt(0)} ${$p("second", 0)}`;
  ...
  toStringShort(useHMS = true, isSpeedrun = false) {
    ...
    if (format(0) === $t("end") && !isSpeedrun) return $t("end");
    ...
    if (totalSeconds > 5e-7 && totalSeconds < 1e-3) {
      ...
      return `${format(1000 * totalSeconds, 0, 3)} ${$t("millisecond")}`;
    }
    if (totalSeconds < 1) {
      ...
      return `${format(1000 * totalSeconds)} ${$t("millisecond")}`;
    }
    if (totalSeconds < 10) {
      return `${format(totalSeconds, 0, 3)} ${$p("second", totalSeconds)}`;
    }
    if (totalSeconds < 60) {
      return `${format(totalSeconds, 0, 2)} ${$p("second", totalSeconds)}`;
    }
    if (this.totalHours < 100 || (isSpeedrun && this.totalHours < 1000)) {
      ...
      if (this.totalMinutes < 60) {
        return `${format(this.totalMinutes, 0, 2)} ${$p("minute", this.totalMinutes)}`;
      }
      if (this.totalHours < 24) {
        return `${format(this.totalHours, 0, 2)} ${$p("hour", this.totalHours)}`;
      }
    }
    if (this.totalDays < 500) {
      return `${isSpeedrun ? this.totalDays.toFixed(2) : format(this.totalDays, 0, 2)} ${$p("day", this.totalDays)}`;
    }
    return `${isSpeedrun ? this.totalYears.toFixed(3) : format(this.totalYears, 3, 2)} ${$p("year", this.totalYears)}`;
  ...
  toTimeEstimate() {
    ...
    if (seconds < 1) return $t("less_than_one_second", formatInt(1));
    if (seconds > 86400 * 365.25) return $t("more_than_one_year", formatInt(1));
    return this.toStringShort();
  }
