...
window.format = function format(value, places = 0, placesUnder1000 = 0) {
  if (isEND()) return $t("end");
  const text = Notations.current.format(value, places, placesUnder1000, 3);
  if (["infinite", "Infinitus", "Primefinity?"].includes(text)) {
    return $t("infinite");
  } else {
    return text;
  }
...
window.makeEnumeration = function makeEnumeration(items) {
  ...
  if (items.length === 2) return $t("X_and_Y", items[0], items[1]);
  const commaSeparated = items.slice(0, items.length - 1).join(", ");
  const last = items[items.length - 1];
  return `${commaSeparated}, and ${last}`;
};

window.$r = function $r(string, values) {
  if (values.length === 0) return string;
  return string.replace(/\$(\d+)/g, function(_, index) {
    const str = values[parseInt(index) - 1];
    if (str instanceof TimeSpan) {
      return str.toString();
    }
    if (typeof str !== "string") {
      throw new Error(`$r Error: Unexpected type ${typeof str} [${str} at index ${index}, string: ${string}]`);
    }
    return str;
  });
};

window.$t = function $t(key, ...values) {
  const string = Languages.current.resources[key]?.(values) ?? Language.en.resources[key]?.(values);
  if (string === undefined) {
    throw new Error(`${key} is not supported in this language.`);
  }
  if (/(\$\d)|(undefined)/.test(string)) {
    throw new Error(`The properties of ${key} is missing or undefined. [string: ${string}]`)
  }
  return string;
};

window.$t_no_split = function $t_no_split(key, ...values) {
  return $t(key, ...values).replace(/\|/g, "");
};

window.$t_split = function $t_split(key, ...values) {
  const string = Languages.current._resources[key] ?? Language.en._resources[key];
  if (string === undefined) {
    throw new Error(`${key} is not supported in this language.`);
  }
  ...
  for (let i = 0; i < splits.length; i++) {
    ...
    if (value !== undefined && value !== null) {
      ...
      const str = $r(split, value);
      if (/(\$\d)|(undefined)/.test(str)) {
        throw new Error(`The properties of ${key} is missing or undefined.[string: ${str}]`)
      }
      splits[i] = str;
    }
  }
...
window.$p = function $p(key, amount, ...values) {
  const one = isSingular(amount);
  const string = Languages.current.plurals[key]?.(one, values) ?? Language.en.plurals[key]?.(one, values);
  if (string === undefined) {
    throw new Error(`${key} is not supported in this language.`)
  }
  if (/(\$\d)|(undefined)/.test(string)) {
...
window.$p_split = function $p_split(key, amount, ...values) {
  return $p(key, amount, ...values).split("|");
};

window.$p_no_split = function $p_no_split(key, ...values) {
  return $p(key, ...values).replace(/\|/g, "");
};

window.$a = function $a(key) {
  const string = Languages.array[key]();
  if (string === undefined) {
    throw new Error(`${key} is not supported in this language.`)
  }
  if (/(\$\d)|(undefined)/.test(string)) {
