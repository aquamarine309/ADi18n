...
class AlchemyResourceState extends GameMechanicState {
  ...
  get name() {
    return $t(`alchemy_resource_${this.id}_name`);
  }
  ...
  get description() {
    return $t(`alchemy_resource_${this.id}_description`);
  }
...
export const AlchemyResource = mapGameDataToObject(
  ...
  (config, id) => (config.isBaseResource
    ? new BasicAlchemyResourceState(config, id)
