...
export const Pelle = {
  ...
  get displayName() {
    return Date.now() % 4000 > 500 ? $t("pelle") : wordShift.randomCrossWords($t("pelle"));
  },
  ...
  armageddon(gainStuff) {
    ...
    player.celestials.nameless.autoStoreReal = false;
  ...
  getSpecialGlyphEffectDescription(type) {
    switch (type) {
      case "infinity":
        return $t("pelle_special_effect_infinity", player.challenge.eternity.current <= 8
  ...
  transitionText(from, to, stage = 0) {
    ...
    for (let i = 0; i < len; i++) {
      if (i >= req) {
        ...
        str += to[idx];
        noZalgo = noZalgo || to[idx].charCodeAt(0) >= 0xff;
        req += toInterval;
      } else {
        const idx = Math.floor(i * (from.length / len));
        str += from[idx];
        noZalgo = noZalgo || from[idx].charCodeAt(0) >= 0xff;
      }
    }
    if (noZalgo) return str;
    return zalgo(str, Math.floor(stage ** 2 * 7));
  },

  get endTabNames() {
    return $t("end_text").split("|");
  },
