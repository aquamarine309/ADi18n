...
export class EternityChallengeState extends GameMechanicState {
  ...
  get maxCompletions() {
    return Nameless.isRunning && this.id === 1 ? 1000 : 5;
  }
  ...
  start(auto) {
    ...
    if (Nameless.isRunning) {
      if (this.id === 6 && this.completions === 5) NamelessProgress.ec6.giveProgress();
      if (!auto && NamelessProgress.challengeCombo.hasProgress) Tab.challenges.normal.show();
    }
