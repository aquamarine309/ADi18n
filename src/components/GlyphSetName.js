export default {
  ...
  computed: {
    ...
    setName() {
      this.sortGlyphList();
      if (this.sortedGlyphs.length === 0) return $t("void_noun");
    ...
    basicTypePhrase() {
      ...
      switch (basicGlyphList.length) {
        case 1:
          return $t(`${basicGlyphList[0].type}_noun`);
        case 2:
        ...
        case 3:
          ...
          if (basicGlyphList[0].perc === basicGlyphList[2].perc) return $t("mixed_irregularity");
        ...
        case 4:
          ...
          if (basicGlyphList[0].perc === basicGlyphList[1].perc) return $t("irregular_jumble");
          return $t("ADJECTIVE_jumble", this.getAdjective(basicGlyphList[0]));
        case 5:
    ...
    singletonName() {
      if (this.sortedGlyphs[0].type === "effarig") return $t_split("effarig_noun")[this.getEffarigProp()];
      const singleGlyphTypes = ["reality", "companion"];
      for (const key of singleGlyphTypes) {
        if (this.sortedGlyphs[0].type === key) return $t(`${key}_noun`);
      }

      // We want a bit of additional flavor for partially-filled sets
      const word = $t(`${this.sortedGlyphs[0].type}_noun`);
      const perc = this.sortedGlyphs[0].perc;
      if (this.isDoomed) return $t("doomed_X", word);
      if (perc === 100) return $t("full_X", word);
      if (perc >= 75) return $t("strengthened_X", word);
      if (perc >= 40) return $("partial_X", word);
      return $t("weak_X", word);
    },
    mainGlyphName() {
  ...
  methods: {
    ...
    getEffarigProp() {
      ...
      if (effarigRM && effarigGlyph) return 0;
      if (effarigRM) return 1;
      if (effarigGlyph) return 2;
      return 3;
    },
    ...
    getAdjective(listEntry) {
      if (listEntry.type === "effarig") return $t_split("effarig_adjectives")[this.getEffarigProp()];
      const adjFn = val => {
        if (val >= 60) return 0;
        if (val >= 40) return 1;
        return 2;
      };
      if (listEntry.type === "companion" || listEntry.type === "reality") {
        return $t(`${listEntry.type}_adjective`);
      }
      const adj = $t_split(`${listEntry.type}_adjectives`);
      return adj[adjFn(listEntry.perc)];
    },
    getNoun(listEntry) {
      if (listEntry.type === "effarig") return $t_split("effarig_noun")[this.getEffarigProp()];
      return $t(`${listEntry.type}_noun`);
    },
  },
  template: `
  <div>
