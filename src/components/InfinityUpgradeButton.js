...
export default {
  ...
  methods: {
    update() {
      ...
      this.worstChallengeString = `(${$t("nc_name", worstChallengeIndex.toString())}: ${timeDisplayShort(worstChallengeTime)})`;
    }
  },
  template: `
  <button
  ...
  >
    <span :class="{ 'o-pelle-disabled': isUseless }">
      ...
      <template v-if="!isDisabled && isImprovedByTS31">
        <br>
        {{ $t("after_X_Y", $t("timestudy_X_short", "31"), formatX(ts31Effect, 2, 2)) }}
      </template>
    </span>
    <CostDisplay
      v-if="!isBought"
