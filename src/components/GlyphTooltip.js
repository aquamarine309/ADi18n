...
export default {
  ...
  computed: {
    ...
    description() {
      const glyphName = $t(this.type === "effarig" ? "effarig" : `${this.type}_noun`);
      switch (this.type) {
        case "companion":
          return $t("TYPE_glyph", $t("companion_noun"));
        case "cursed":
          return $t("cursed_glyph");
        case "reality":
          return $t("RARITY_glyph_of_TYPE", $t("pure"), glyphName);
        default:
          return $t("RARITY_glyph_of_TYPE", $t_split("glyph_rarities")[this.rarityInfo.id], glyphName);
      }
    },
    isLevelCapped() {
      return this.displayLevel && this.displayLevel < this.level;
    ...
    levelText() {
      ...
      const color = this.isLevelCapped
        ? "#ff4444"
        : (this.isLevelBoosted ? "#44FF44" : null);
      return $t("X_colon_Y", $t("level"), `<span style="color: ${color}">
  ...
  methods: {
    ...
    sacrificeText() {
      ...
      return `<span style="font-weight: ${isCurrentAction ? "bold" : ""};">
              ${$t("X_colon_Y", $t("sacrifice"), powerText)}
              </span>`;
    },
    refineText() {
      if (this.type === "companion" || this.type === "cursed" || this.type === "reality") return "";
      if (!AlchemyResource[this.type].isUnlocked) return "";
      const refinementText = `${format(this.uncappedRefineReward, 2, 2)} ${GLYPH_SYMBOLS[this.type]}`;
      let additionalText = "";
      if (this.uncappedRefineReward !== this.refineReward) {
        additionalText = `(${$t("actual_value_due_to_cap_X_TYPESYMBOL", format(this.refineReward, 2, 2), GLYPH_SYMBOLS[this.type])})`;
      }
      const isCurrentAction = this.currentAction === "refine";
      return `<span style="font-weight: ${isCurrentAction ? "bold" : ""};">
              ${$t("refine_X_TYPESYMBOL", refinementText, additionalText)}
              </span>`;
    },
    scoreText() {
      if (this.type === "companion" || this.type === "cursed" || this.type === "reality") return "";
      ...
      return $t("score_X", format(AutoGlyphProcessor.filterValue(this.$parent.glyph), 1, 1));
    }
  },
  template: `
  <div
