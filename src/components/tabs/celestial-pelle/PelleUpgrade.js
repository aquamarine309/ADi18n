...
export default {
  ...
  computed: {
    ...
    effectText() {
      ...
      const prefix = this.isCapped ? `${$t("capped")}:` : `${$t("currently")}:`;
    ...
    timeEstimate() {
      ...
      if (this.notAffordable) return "生成星系的上限导致你不能购买此升级";
    ...
    hasTimeEstimate() {
      return !(this.canBuy ||
        this.isBought ||
        this.isCapped ||
        (this.galaxyGenerator && this.config.id <= 1)
      );
    },
  ...
  >
    ...
    <CostDisplay
      v-if="!isCapped"
      :config="config"
      :name="galaxyGenerator ? config.currencyLabel : 'reality_shards'"
      data-v-pelle-upgrade
    />
  </button>
  `
