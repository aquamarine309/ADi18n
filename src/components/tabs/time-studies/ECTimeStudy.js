...
export default {
  ...
  computed: {
    ...
    name() {
      return $t("ec_name_short", this.id.toString());
    }
  },
  methods: {
    update() {
      ...
      this.showTotalCompletions = !Nameless.isRunning || id !== 1;
  ...
  <TimeStudyButton :setup="setup">
    ...
    <template v-if="hasRequirement">
      ...
      <span v-if="config.secondary.path">{{ config.secondary.path() }}</span>
      <span v-else>
    ...
    <span v-else-if="isRunning"><br>{{ $t("currently_running") }}</span>
  </TimeStudyButton>
  `
};