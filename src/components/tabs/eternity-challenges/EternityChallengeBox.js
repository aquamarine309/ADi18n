...
export default {
  ...
  computed: {
    ...
    goalDisplay() {
      ...
      let goal = `${$t("goal")}: ${this.goalAtCompletions(this.completions)} ${$t("infinity_points_short")}`;
  ...
  methods: {
    update() {
      ...
      this.lastGoal = (Nameless.isRunning && this.challenge.id === 1)
  ...
  >
    ...
    <template #bottom>
      <div :style="{ visiblity: completions < 5 ? 'visible' : 'hidden' }">
        <div>
          {{ $p("ec_completed_times", completions, formatInt(completions)) }}
        </div>
        {{ goalDisplay }}
      </div>
      <span v-if="showGoalSpan">
        {{ $t("goal_span") }}: {{ firstGoal }} {{ $t("infinity_points_short") }} - {{ lastGoal }} {{ $t("infinity_points_short") }}
      </span>
      <span>
        {{ $t("reward") }}:
        <DescriptionDisplay
