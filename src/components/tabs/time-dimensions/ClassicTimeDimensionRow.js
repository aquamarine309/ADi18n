...
export default {
  ...
  computed: {
    ...
    name() {
      return $t(`_${TimeDimension(this.tier).shortDisplayName}_td`);
    },
    buttonContents() {
    ...
    formattedTTCost() {
      return $t("unlock_X_tt", format(this.ttCost));
    },
    formattedEPCost() {
      if (this.isCapped) return $t("capped");
      if (this.showCostTitle) return $t("cost_X_eternity_points_short", format(this.cost, 2));
      return `${format(this.cost, 2)} ${$t("eternity_points_short")}`;
    },
    hasLongText() {
  ...
  methods: {
    update() {
      ...
      this.isCapped = Nameless.isRunning && dimension.bought > 0;
