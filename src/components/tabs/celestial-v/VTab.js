...
export default {
  ...
  computed: {
    ...
    runDescription() {
      return GameDatabase.celestials.descriptions[3].effects().map(d => d.replace(/^\w/u, c => c.toUpperCase()));
    },
  ...
  methods: {
    ...
    startRun() {
      ...
      Modal.celestials.show({ name: $t("v_reality"), number: 3 });
    },
    has(info) {
    ...
    <div v-else>
      ...
      >
        <PrimaryButton
          ...
          @click="toggleFlipped"
        >
          <span v-if="wantsFlipped">{{ $t("hide_hard_v") }}</span>
          <span v-else>{{ $t("show_hard_v") }}</span>
        </PrimaryButton>
        <PrimaryButton
      ...
      <div
        ...
        class="c-v-info-text"
      >
        {{ $p_no_split("you_have_X_perk_points", pp, formatInt(pp, 2, 0)) }}
      </div>
        ...
        >
          ...
          >
            ...
            <p class="o-v-unlock-amount">
              {{ $t("X_Y_done", formatInt(hex.completions), formatInt(hex.config.values.length))}}
            </p>
            <div v-if="showRecord(hex)">
              <p class="o-v-unlock-record">
                {{ $t("best_X", hex.config.formatRecord(runRecords[hex.id])) }}
              </p>
              <p>
                <GlyphSetPreview
          ...
          >
            ...
            >
              ...
              <span v-else>{{ $t("start_X", $t("v_reality")) }}</span>
            </b>
            <br>
            <div
              :style="{ 'font-size': hasAlchemy ? '1.2rem' : '' }"
              v-for="(text, i) in runDescription"
              :key="i + 'run'"
            >
              {{ text }}
            </div>
        ...
        >
          ...
          >
            <div :class="{ 'o-pelle-disabled': isDoomed }">
              ...
              <p>{{ $t("reward") }}: {{ milestone.rewardText }}</p>
              <p v-if="milestone.formattedEffect">
                {{ $t("currently") }}: <b>{{ milestone.formattedEffect }}</b>
              </p>
            </div>
          </div>
        </div>
