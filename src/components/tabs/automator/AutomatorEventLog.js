    ...
    timestamp(entry) {
      switch (this.timestampMode) {
        ...
        case AUTOMATOR_EVENT_TIMESTAMP_MODE.THIS_REALITY:
          return `, ${$t("X_real_time_in_reality", TimeSpan.fromSeconds(entry.thisReality).toStringShort())}`;
        case AUTOMATOR_EVENT_TIMESTAMP_MODE.RELATIVE_NOW:
          return `, ${$t("X_ago", TimeSpan.fromMilliseconds(this.currentTime - entry.timestamp).toStringShort())}`;
        case AUTOMATOR_EVENT_TIMESTAMP_MODE.RELATIVE_PREV:
          if (entry.timegap === entry.timestamp) return `, first logged event`;
          return `, ${$t("X_after_previous_event", TimeSpan.fromMilliseconds(entry.timegap).toStringShort())}`;
