...
export default {
  ...
  computed: {
    ...
    completionsDisplay() {
      ...
      return $p("X_completions", this.completions, `${formatInt(this.completions)}/${maxStr}`);
    },
    progressDisplay() {
      ...
      switch (this.milestoneMode) {
        case SINGULARITY_MILESTONE_RESOURCE.SINGULARITIES:
          return $p("in_X_singularities", this.remainingSingularities, format(this.remainingSingularities, 2));
        case SINGULARITY_MILESTONE_RESOURCE.CONDENSE_COUNT:
          return $p("condense_X_times", condenseCount, format(condenseCount, 2, 2));
        case SINGULARITY_MILESTONE_RESOURCE.MANUAL_TIME:
          ...
          return $t("in_X_manual", TimeSpan.fromSeconds(thisSingularityTime + extraTime).toStringShort());
        case SINGULARITY_MILESTONE_RESOURCE.AUTO_TIME:
          ...
          timeText = $t("in_TIME", TimeSpan.fromSeconds(thisSingularityTime + extraTime).toStringShort());
