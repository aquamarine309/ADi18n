export default {
  ...
  computed: {
    ...
    intervalText() {
      ...
      const str = interval > 1000 ? `${format(interval / 1000, 2, 2)}s` : `${format(interval, 2, 2)}${$t("millisecond")}`;
      const line1 = this.hoverOverAscension ? `<b>${str}</b>` : str;

      let line2;
      if (this.isIntervalCapped) line2 = this.hoverOverAscension ? "On ascend ➜" : $t("ascend");
      else line2 = $t("cost_X_dark_matter_short", this.formatDMCost(this.intervalCost));
    ...
    darkMatterText() {
      ...
      const line2 = this.hoverOverAscension
        ? `${formatX(ascMult, 2, 2)} / ${$t("second_short")}`
        : $t("cost_X_dark_matter_short", this.formatDMCost(this.powerDMCost));
      return `${line1}<br>${line2}`;
    },
    darkEnergyText() {
      const de = this.powerDE * (this.hoverOverAscension ? POWER_DE_PER_ASCENSION : 1);
      const str = `${$t("dark_energy_short")} +${format(de, 2, 4)}`;
      const line1 = this.hoverOverAscension ? `<b>${str}</b>` : str;
      const ascMult = POWER_DE_PER_ASCENSION * this.interval / this.intervalAfterAscension;
      const line2 = this.hoverOverAscension
        ? `${formatX(ascMult, 2, 2)} / ${$t("second_short")}`
        : $t("cost_X_dark_matter_short", this.formatDMCost(this.powerDECost));
      return `${line1}<br>${line2}`;
    },
    ascensionTooltip() {
  ...
  >
    ...
    <div>
      {{ $t("average_gain", format(productionPerSecond, 2, 2)) }}/s
    ...
    <div v-if="interval > 200">
      {{ $t("tick_X", formatInt(timer)) }} {{ $t("millisecond") }} ({{ formatPercents(timerPercent, 1) }})
    </div>
    <div v-else>
      {{ $t("X_ticks_per_sec", format(1000 / interval, 2, 2)) }}
    </div>
    <div>
      {{ $t("dark_energy_X", format(darkEnergyPerSecond, 2, 4)) }}/s ({{ $t("X_of_total", formatPercents(portionDE, 1)) }})
    </div>
  </div>
  `
};