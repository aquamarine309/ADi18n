...
export default {
  ...
  computed: {
    ...
    uniqueGlyphText() {
      ...
      if (this.hasEffarig) uniqueGlyphs.push(
        `<span style="color: ${GlyphAppearanceHandler.getBorderColor("effarig")};">${$t("effarig")}</span>`);
      if (this.hasReality) uniqueGlyphs.push(
        `<span style="animation: a-reality-glyph-description-cycle 10s infinite;">${$t("reality_noun")}</span>`);
    ...
    pelleGlyphText() {
      return Pelle.isDoomed
        ? $t("glyph_cap_pelle", formatPercents(strengthToRarity(Pelle.glyphStrength)), formatInt(Pelle.glyphMaxLevel))
        : "";
    },
    showChaosText() {
  ...
  <div class="c-current-glyph-effects l-current-glyph-effects">
    ...
    <div class="c-current-glyph-effects__header">
      {{ $t("currently_active_glyph_effects") }}
    </div>
    ...
    >
      <span class="c-current-glyph-effects__effect--capped">{{ $t("glyph_effects_italic_info") }}</span> 
    </div>
    <br>
    <div v-if="noEffects">
      {{ $t("no_glyph_effects") }}
