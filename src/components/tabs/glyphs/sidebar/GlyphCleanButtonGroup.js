export default {
  ...
  computed: {
    removeString() {
      if (this.isRefining) return $t("refine");
      if (this.glyphSacrificeUnlocked) return $t("scrifice");
      return $t("delete");
    },
    autoCleanTooltip() {
  ...
  >
    ...
    >
      {{ $t("sacrifice_all_unprotected_glyphs") }}
    </button>
    <button
      v-if="hasFilter"
      class="c-glyph-inventory-option"
    ...
    >
      {{ $t("sacrifice_all_glyphs_rejected_by_filtering") }}
      <div
