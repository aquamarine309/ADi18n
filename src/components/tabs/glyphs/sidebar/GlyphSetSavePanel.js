...
export default {
  ...
  computed: {
    ...
    noSet() {
      return $t("no_glyph_preset_saved_in_this_slot");
    },
  },
  watch: {
    effects(newValue) {
  ...
  methods: {
    ...
    setName(id) {
      ...
      return $t("glyph_preset_ID_NAME", (id + 1).toString(), name);
    },
    saveGlyphSet(id) {
    ...
    loadGlyphSet(set, id) {
      ...
      if (missingGlyphs > 0) {
        GameUI.notify.error($t("glyph_set_missing_glyphs", missingGlyphs, this.setName(id)));
      } else {
        GameUI.notify.success($t("glyph_set_loaded", this.setName(id)));
      }
    },
