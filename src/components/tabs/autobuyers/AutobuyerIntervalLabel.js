...
export default {
  ...
  computed: {
    intervalText() {
      ...
      if (this.interval < player.options.updateRate && this.autobuyer.name !== "Infinity") return `当前间隔: ${$t("instant")}`;
      return $p("current_interval", this.interval, format(TimeSpan.fromMilliseconds(this.interval).totalSeconds, 2, 2));
    },
    bulkText() {
